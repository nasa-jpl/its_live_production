...
echo '
=======================================================
Building virtual datacube docker image...
======================================================='

set -ex

IMAGE='its_live/icechunk_datacube'
TAG=$1
...
cp -r ${WORKSPACE}/src/virtual_itslive_cube_per_chunk.py \
      ...
      ${WORKSPACE}/src/shapefile.py \
      ${WORKSPACE}/src/itscube_types.py \
      ${WORKSPACE}/src/itslive_binary_type.py \
      ${WORKSPACE}/src/grid.py \
      ${WORKSPACE}/src/itslive_mosaics_types.py \
      ${WORKSPACE}/LICENSE \
      ${TEMP_STAGING_DIR}/

cp -r ${WORKSPACE}/environment/virtual_cube_environment.yml \
      ${WORKSPACE}/environment/aws_config \
      ${WORKSPACE}/docker/entrypoint_virtual_cube.sh \
...
docker build --no-cache --rm --force-rm -t ${IMAGE}:${TAG} \
    ...
    -f ${WORKSPACE}/docker/Dockerfile_virtual_datacube ${WORKSPACE}
