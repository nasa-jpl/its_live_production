#!/bin/bash
#
# Script to build ITS_LIVE docker image for the granules kerchunk references.
#
echo '
=======================================================
Building kerchunk-ref docker image...
======================================================='

set -ex

IMAGE='its_live/kerchunk_ref'
TAG=$1
...
TEMP_STAGING_DIR=$(mktemp -d staging_for_kerchunk_docker_image_XXXXXXXXXX)
function cleanup {
...
cp -r ${WORKSPACE}/src/kerchunk/gen_refs.py \
      ${WORKSPACE}/LICENSE \
      ${TEMP_STAGING_DIR}/

cp -r ${WORKSPACE}/environment/kerchunk_environment.yml \
      ${WORKSPACE}/docker/entrypoint_kerchunk.sh \
...
docker build --rm --force-rm -t ${IMAGE}:${TAG} \
    ...
    -f ${WORKSPACE}/docker/Dockerfile_kerchunk ${WORKSPACE}
