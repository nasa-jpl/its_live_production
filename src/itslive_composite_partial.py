...
This version of the code rebuilds only the partial segment of the existing
composite. This is to address corrupted composites that seem
to be generated on some of the EC2 platforms.
...
class CompositeVariable:
    ...
    def transpose(self, dims=CONT_IN_X):
        ...
        Transpose data variables to new dimensions.
    ...
    def getThreeDimData(name, ds, x_index):
        """Extract data values for the range of interest from existing
            xr.DataArray. The dimensions of xr.DataArray are in (time, y, x)
            order while returned data should be in (y, x, time) dimensions.

        Args:
            name (str): Name of the xr.DataArrat data variable
            ds (xr.Dataset): Existing composite dataset.
            x_index (int): X index for the [0, x] range to extract the data for.
    ...
    def setThreeDimData(self, ds, x_index, vars_names):
        """For each of the provided data variables names extract data values
            for the range of interest from existing xr.DataArray and set
            corresponding data variable values.
            The dimensions of each xr.DataArray are in (time, y, x)
            order while data to be set should be in (y, x, time) dimensions.

        Args:
            ds (xr.Dataset): Existing composite dataset.
            x_index (int): X index for the [0, x] range to extract the data for.
            vars_names (str): Names of the xr.DataArray data variables to
                extract and set to the corresponding data variables.  If None
                value is provided, then skip setting that data variable.
        """
        for each_name, each_var in zip(
            vars_names, [self.v, self.vx, self.vy]
        ):
            if each_name:
                # Name of the data variable is provided
                each_var[:, :x_index, :] = CompositeVariable.getThreeDimData(
                    each_name, ds, x_index
                )

    @staticmethod
    def getTwoDimData(name, ds, x_index):
        """Extract data values for the range of interest from existing
            xr.DataArray. The dimensions of xr.DataArray are in (y, x)
            order which is the same as the target numpy array dimentions are.
    ...
    def setTwoDimData(self, ds, x_index, vars_names):
        ...
        for each_name, each_var in zip(
            ...
            if each_name:
                # Name of the data variable is provided
                each_var[:, :x_index] = CompositeVariable.getTwoDimData(
                    each_name, ds, x_index
                )
...
class ITSLiveComposite:
    ...
    # Original data is stored in [time, y, x] dimension order.
    ...
    # X index to indicate the [:, : 0:X] range for which to keep data from
    # already existing composite
    X_KEEP = 0

    def __init__(self, cube_store: str, s3_bucket: str, composite_store: str):
        """
        ...
        composite_store: Filename for the composite. This is an existing
            composite on the local filesystem that will have its partial
            update.
        """
        # Populate composite's data with existing values from X/Y section that is
        # not being updated: [:, x_start)
        # Make sure to "revert" updated values for all data variables -
        # it's done right before final composite is written to the file.
        ...
        if self.sensor_filter.excludeS2FromLSQ:
            ...
            # These are not output to the target composites, so no need
            # to populate these with existing data as there is none exists.
...
if __name__ == '__main__':
    ...
    ITSLiveComposite.X_KEEP = args.xStartIndex
