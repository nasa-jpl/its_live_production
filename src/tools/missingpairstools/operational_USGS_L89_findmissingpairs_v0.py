...
parser.add_argument('-geojson_cat_dir', default='/Users/mark/itslive/catalog_geojson/landsatOLI/v02',help='directory with geojson catalogs (synced with same dir on itslive AWS) [%(default)s]')
...
#
# ONLY fetch new version if older that 2 days to limit copying if script run multiple times
#
...
for injsonfile in tqdm(injsonlist,desc="Reading Geojson Catalogs"):
#     print(f'{injsonfile}')
...
for pair_name in tqdm(existing_pairs_files,desc="Parsing Existing Pairs"):
...
#             delt = (dt_img1 - dt_img2).days
            ...
            if delt < min_delt:
                min_delt = delt
            if args.v and delt%8 != 0 and delt<16:  #  (this justs points out exising odd short dts from commissioning phase, doesn't remove them)(%8 allows 8 day L8xL9)
...
for pair_name in tqdm(existing_pairs_files,desc="building existing pairs dictionary"):
    split_name = pair_name.split('_')
...
#         img1_key = '_'.join(split_name[0:4])
        ...
        if img2_key in img_dict[pr_key].keys():
            if delt in img_dict[pr_key][img2_key].keys():
                ...
                if args.v:
                    print(f'>>>>>>>>>>>>>>>>>>>>>>>>>>>> second existing pair with same date/dt? {pair_name}')
            else:
...
usgs_imagelists_offnadir_scenes_dict = {}
...
rt_or_lt_count = 0
#row = next(reader)
for row in tqdm(reader,desc="Reading USGS current image list"):
    counter += 1
#     if counter % 100000 == 0:
#         print(f'scanned {counter} L8/9 scenes so far')
    if not(row['Collection Category'] == 'RT') and not(row['Landsat Product Identifier L1'][:2] == 'LT'):  # skip near Real Time and Thermal only acquisitions
        if row['Nadir/Off Nadir'] == 'NADIR':
            ...
            # check if off_nadir scene is of ice PR (in northern Greenland and southern Antarctica these may yield velocity pairs)- keep it if it is
            off_nadir += 1
            fields = row['Landsat Product Identifier L1'].split('_') # 'LC08_L1TP_087231_20200604_20200604_01_T1'

            if fields[2] in img_dict.keys(): # a path/row we want
                img_short = '_'.join(fields[:4])
                if fields[2] not in usgs_imagelists_offnadir_scenes_dict.keys():
                    usgs_imagelists_offnadir_scenes_dict[fields[2]] = {}
                if img_short not in usgs_imagelists_offnadir_scenes_dict[fields[2]]:
                    usgs_imagelists_offnadir_scenes_dict[fields[2]][img_short] = row
                    off_nadir_ice += 1
                else:
                    print(f"DUPLICATE IMAGE in USGS list {usgs_imagelists_dict[fields[2]][img_short]['Landsat Product Identifier L1']} and {row['Landsat Product Identifier L1']}")
...
print(f'scanned {counter} L8 scenes: off_nadir {off_nadir}  off_nadir but ice {off_nadir_ice} rt_or_lt_count {rt_or_lt_count}')
...
for pathrow in tqdm(usgs_needed_pairs_dict,desc="finding missing pairs"):

    temp_usgs_lowcloud = [] # will be a list of image date strings for this pathrow
    ...
    for image in usgs_imagelists_dict[pathrow]:
        if float(usgs_imagelists_dict[pathrow][image]['Land Cloud Cover']) <= cloudfraction_cutoff:
            temp_usgs_lowcloud.append((image.split('_')[3],image)) # image date from short name, short name

    temp_usgs_lowcloud.sort(reverse=True) # reverse sort by date
    usgs_lowcloud = [x[1] for x in temp_usgs_lowcloud] # keep reverse sorted list of short names, drop datestr field


    # ASSUMES there is at least one pair - not checked because it is true here - might check in off_nadir section below
    usgs_needed_pairs_dict[pathrow] = {x:{} for x in usgs_lowcloud}

    for index,scene in enumerate(usgs_lowcloud[:-1]): # skip oldest (list reverse sorted by date already)
        dt_img2 = dt.datetime.strptime(scene.split('_')[3],"%Y%m%d")
        n=index+1
        dt_img1 = dt.datetime.strptime(usgs_lowcloud[n].split('_')[3],"%Y%m%d")
        delt = (dt_img2 - dt_img1).days
        # iterate over the rest of the list to form all possible pairs
        while n < len(usgs_lowcloud) and delt <= max_time_separation_days:
            ...
            if not found_match: # record this pair as needing to be processed
                if delt in usgs_needed_pairs_dict[pathrow][scene].keys(): # already have this one?
                    if args.v:
                        print(f'>>>>>>>>>>>>>>>>>>>>> duplicate pair called for {usgs_needed_pairs_dict[pathrow][scene][delt]} and {temp_pair_dict}')
                else:
...
########
#
# now off_nadir scenes
#
########

off_nadir_to_process = 0
off_nadir_already_have = 0
for pathrow in usgs_imagelists_offnadir_scenes_dict:

    temp_usgs_lowcloud = [] # will be a list of image date strings for this pathrow

    for image in usgs_imagelists_offnadir_scenes_dict[pathrow]:
        if float(usgs_imagelists_offnadir_scenes_dict[pathrow][image]['Land Cloud Cover']) <= cloudfraction_cutoff:
            temp_usgs_lowcloud.append((image.split('_')[3],image)) # image date from short name, short name
...
#    usgs_needed_pairs_dict[pathrow] will already be there - need to add new scenes in here
    
    if len(usgs_lowcloud) > 1: # pairs might be made
        for scene in usgs_lowcloud: # add dict entry for all the possible new scenes in this PR
            usgs_needed_pairs_dict[pathrow][scene] = {}
        for index,scene in enumerate(usgs_lowcloud[:-1]): # skip oldest (list reverse sorted by date already)
            dt_img2 = dt.datetime.strptime(scene.split('_')[3],"%Y%m%d")
            n=index+1
            dt_img1 = dt.datetime.strptime(usgs_lowcloud[n].split('_')[3],"%Y%m%d")
            ...
            while n < len(usgs_lowcloud) and delt <= max_time_separation_days:
                temp_pair_dict = {
                                'img1':usgs_imagelists_offnadir_scenes_dict[pathrow][usgs_lowcloud[n]]['Landsat Product Identifier L1'],
                                'img2':usgs_imagelists_offnadir_scenes_dict[pathrow][scene]['Landsat Product Identifier L1']
                                }
                found_match = False
                if not(args.ignore_existing_itslive_pairs): # skip this section if we are creating all possible combinations and ignoring what is in archive already
                    if scene in img_dict[pathrow].keys() and delt in img_dict[pathrow][scene].keys():
                        ...
                        for c in posspairs:  # iterate over versions for this delt pair if they are there
                            ...
                            if img_short_name(posspairs[c]['img1']) == img_short_name(temp_pair_dict['img1']) and \
                                        img_short_name(posspairs[c]['img2']) == img_short_name(temp_pair_dict['img2']):
                                found_match = True
                                off_nadir_already_have += 1
                if not found_match: # record this pair as needing to be processed
                    if delt in usgs_needed_pairs_dict[pathrow][scene].keys(): # already haave this one?
                        if args.v:
                            print(f'>>>>>>>>>>>>>>>>>>>>> duplicate pair called for {usgs_needed_pairs_dict[pathrow][scene][delt]} and {temp_pair_dict}')
                    else:
                        usgs_needed_pairs_dict[pathrow][scene][delt] = temp_pair_dict
                        total_needed_pairs += 1
                        off_nadir_to_process += 1
                n += 1
                if n<len(usgs_lowcloud):
                    dt_img1 = dt.datetime.strptime(usgs_lowcloud[n].split('_')[3],"%Y%m%d")
                    delt = (dt_img2 - dt_img1).days

print(f'ADDED OFF NADIR: off_nadir_already_have {off_nadir_already_have}   off_nadir_to_process {off_nadir_to_process}')
print(f'found {total_needed_pairs} missing/new scene pairs to process; its_live has {same_pr_delt_first_occurence} or {existing_its_live_pairs_same_prs - same_pr_duplicates} (non-duplicate) ({same_pr_duplicates} same_pr_delt_duplicates) L8-only same path/row pairs already')
if args.log_numbers_file:
    logfile.write(f'found {total_needed_pairs} missing/new scene pairs to process; its_live has {same_pr_delt_first_occurence} or {existing_its_live_pairs_same_prs - same_pr_duplicates} (non-duplicate) ({same_pr_duplicates} same_pr_delt_duplicates) L8-only same path/row pairs already')
...
with open(f'L89_pairs_to_process_{dt.datetime.now().strftime("%Y_%m_%d")}.txt','w') as outfile:
    for (img1,img2) in tqdm(image_pair_list,desc=f"writing L89_pairs_to_process_{dt.datetime.now().strftime('%Y_%m_%d')}.txt"):
