...
1. Use original granules from s3://its-live-project/velocity_image_pair/sentinel1-latest
2. Copy M11 and M12 values into target granule, preserve encoding values
3. Copy granule with restored M11 and M12 into destination s3://its-live-data/velocity_image_pair/sentinel1-restoredM
...
class RestoreM11M12Values:
    ...
    Restore original M11 and M12 values for uncorrected V2 Sentinel-1 granules (that are residing in AWS S3 bucket):
    1. Use original granules from s3://its-live-project/velocity_image_pair/sentinel1-latest
    2. Crop original granule's M11 and M12 values to valid X/Y extends for non-zero coverage granules
        (since target granule is already cropped)
    ...
    def __init__(self, glob_pattern: dir, granules_to_exclude: str):
        ...
        # will be will be provided by Joe as input parameter files.
        ...
        if len(self.exclude_granules):
            # ATTN: Since paths are changed for some of the cropped granules in the target directory, it will not match
            # such granules full paths when excluding, which is OK as we will attempt to restore M11/M12 but won't be
            # able to. Second pass of M11/M12 restoration when using M11/M12 input parameter files generated by Joe will
            # take care of such granules.
