...
bucket="s3://its-live-data/mosaics/annual/v2_slow_error/fixed_sensor_flag"
...
for filename in $(awsv2 s3 ls "$bucket"/ | grep .nc | grep -v 0000 | grep -v 2023 | awk '{print $NF}'); do
    # Copy file locally
