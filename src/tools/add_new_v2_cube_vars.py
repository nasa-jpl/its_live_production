#!/usr/bin/env python
"""
Add new data variables to the V2 ITS_LIVE datacubes that are residing in AWS S3 bucket:
...
* Push modified datacubes back to the S3 bucket.

This functionality might be combined with the tool that restores M11/M12 values for S1 layers of the
datacubes.
...
class AddDatacubesVariables:
    ...
    * github issue #27: Using granules img_pair_info variable, add binary array for 'ascending/descending' per each image
    of the granule:
        ascending_img1 = [true/false]
        ascending_img2 = [true/false]

    * github issue #28: Add new sensor_uid1 & sensor_uid2 data variables of uint8 type to datacubes.
        Both variables should have description of unique values:
            description: unique sensor id: 4 = Landsat 4, 5 = Landsat 5, 6 = Landsat 6, 7 = Landsat 7, 8 = Landsat 8,
            9 = Landsat 9, 11 = Sentinel 1A, 12 = Sentinel 1B, 21 = Sentinel 2A, 22 = Sentinel 2B

    * Push corrected datacubes back to the S3 bucket - should be new target location or would need to delete original
    datacubes before pushing updated datacubes.
    """
    S3_PREFIX = 's3://'
    DRY_RUN = False
    ...
    def __init__(self, bucket: str, bucket_dir: str, target_bucket_dir: str, local_original_cube_dir: str, local_dir: str):
        ...
        Args:
            ...
            target_bucket_dir (str): AWS S3 directory to store corrected datacubes.
        ...
        # Process one cube for debugging only
    ...
    def debug__call__(self, num_dask_workers: int):
        ...
        for each_cube in self.all_zarr_datacubes:
            ...
            msgs = AddDatacubesVariables.all(
                each_cube,
                self.bucket_dir,
    ...
    def __call__(self, num_dask_workers: int, start_cube: int = 0):
        """
        Add new data variables to V2 datacubes.
        """
        ...
        while num_to_fix > 0:
            ...
            tasks = [
                dask.delayed(AddDatacubesVariables.all)(
                    each,
                    self.bucket_dir,
                    self.target_bucket_dir,
                    self.local_original_cube_dir,
    ...
    def all(
        ...
        """
        Add new variables to the datacubes and copy them to S3 bucket's new target destination.
        ...
        with xr.open_dataset(local_original_cube, decode_timedelta=False, engine='zarr', consolidated=True) as ds:
            # Add unique sensor ID variables per each image in the granule
            for each_var in [
            ...
            # Add ascending_img[12] data variables to the cube with data available only for S1 layers
            ...
            msgs.append(f'Identified {np.sum(s1_mask)} S1 layers in the cube, extract img_pair_info.flight_direction_img[12] values')
            ...
            for each_index in mask_i[0]:
                ...
                # ATTN: If using new temporary location of restored S1 granules, remove this once all S1 granules
                # are in their target location
            ...
            # Correct chunking settings in the cube, use them as golden standard for the rest of the variables
            ...
            # If not already fixed by other "correction" tool:
            ...
            # Chunking for X and Y are set to full extend by default, set it explicitely just to be sure
        ...
        if AddDatacubesVariables.DRY_RUN:
            return msgs
        ...
        if os.path.exists(fixed_file):
            ...
            if not target_url.startswith(AddDatacubesVariables.S3_PREFIX):
                target_url = AddDatacubesVariables.S3_PREFIX + target_url
...
def main():
    ...
    parser.add_argument(
        ...
        help='AWS S3 that stores ITS_LIVE datacubes to add new data variables to [%(default)s]'
    )
    parser.add_argument(
        '-d', '--bucket_dir',
        type=str,
    ...
    AddDatacubesVariables.DRY_RUN = args.dryrun

    fix_cubes = AddDatacubesVariables(
        args.bucket,
        args.bucket_dir,
