...
class ProcessV2Granules:
    ...
    def __init__(self, granule_list):
        ...
        self.all_granules = granule_list
    ...
    def __call__(self, start_index: int = 0, stop_index: int = -1):
        """
        Crop V2 ITS_LIVE granules to X/Y extend of non-None values or
        copy zero percent coverage files over to new location.
        """
        if stop_index > 0:
            self.all_granules = self.all_granules[:stop_index]
            logging.info(f'Stop index ({stop_index}) is provided, leaving {len(self.all_granules)} to process')
    ...
    def crop(granule_url: str, s3: s3fs.S3FileSystem):
        ...
        with s3.open(granule_url) as fhandle:
            with xr.open_dataset(fhandle) as ds:
                ...
                for _, each_var_settings in granule_encoding.items():
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help='Index for the stop granule to process (if splitting processing b/w multiple EC2s) [%(default)d]'
    )
    parser.add_argument(
        '-m', '--mission',
        type=str,
    ...
    parser.add_argument(
        ...
        help=f'Read granule file list to avoid time consuming glob of S3 bucket.'
    )
    parser.add_argument(
        '--skip_granule_list',
        type=str,
        default='l7_corrected_granules.json',
        help=f'Read a list of granules to skip (these granules are corrected by a separate processing to address wrong pvalue). '
    )
    ...
    existing_granules = []
    with open(args.granule_list, 'r') as fh:
        existing_granules = json.load(fh)

    logging.info(f"Loaded {len(existing_granules)} granules from '{args.granule_list}'")

    skip_granules = []
    with open(args.skip_granule_list, 'r') as fh:
        skip_granules = json.load(fh)

    logging.info(f"Loaded {len(skip_granules)} granules from '{args.skip_granule_list}'")

    granules_to_crop = list(set(existing_granules).difference(tuple(skip_granules)))
    logging.info(f"{len(granules_to_crop)} granules to crop")

    process_granules = ProcessV2Granules(granules_to_crop)
