...
For all S1 granules replace v.description attribute (originally set to
'velocity magnitude from radar range and azimuth measurements') with 'velocity magnitude'.
...
class ProcessV2Granules:
    ...
    def copy(granule_url: str, s3: s3fs.S3FileSystem):
        ...
        with s3.open(granule_url) as fhandle:
            with xr.open_dataset(fhandle) as ds:
                ds = ds.load()
                ds[DataVars.V].attrs[DataVars.DESCRIPTION_ATTR] = 'velocity magnitude'
                ...
                bucket_granule = granule_url.replace(ProcessV2Granules.SOURCE_DIR, ProcessV2Granules.TARGET_DIR)
                bucket_granule = bucket_granule.replace(ProcessV2Granules.BUCKET+'/', '')

                # Upload corrected granule to the bucket's new location
                s3_client = boto3.client('s3')
                try:
                    msgs.append(f"Uploading to {bucket_granule}")
...
def main():
    ...
    parser.add_argument(
        '-t', '--target_bucket_dir',
        type=str,
        default='velocity_image_pair/sentinel1-new/v02',
        help='AWS S3 bucket and directory that store the granules'
    )
    parser.add_argument(
        '-l', '--local_dir',
