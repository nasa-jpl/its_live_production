...
class NSIDCMosaicFormat:
    ...
    # Flag to indicate if the files should be overwritten if they
    # already exist in the target directory in s3 bucket
    ...
    def process_file(target_bucket: str, infilewithpath: str, s3: s3fs.S3FileSystem, target_dir: str):
        ...
        if NSIDCMosaicFormat.OVERWRITE_EXISTING_FILES is False:
            if NSIDCFormat.object_exists(bucket, bucket_granule):
                ...
                # Premet file
                bucket_granule = os.path.join(
                    target_dir,
                    f'{filename}{NSIDCMeta.PREMET_EXT}'
                )
                ...
                if NSIDCFormat.object_exists(bucket, bucket_granule):
                    ...
                    bucket_granule = os.path.join(
                        target_dir,
                        f'{filename}{NSIDCMeta.SPATIAL_EXT}'
                    )
        ...
        with xr.open_dataset(local_file, engine=NSIDCMeta.NC_ENGINE) as ds:
            ...
            # Convert dataset to Dask dataset not to run out of memory
    ...
    def process_nc_file(
        ...
        ds[ShapeFile.LANDICE].encoding[Output.FILL_VALUE_ATTR] = None

        del ds[ShapeFile.FLOATINGICE].encoding[Output.MISSING_VALUE_ATTR]
        ds[ShapeFile.FLOATINGICE].encoding[Output.FILL_VALUE_ATTR] = None
