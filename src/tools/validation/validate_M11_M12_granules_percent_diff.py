"""
Script to collect statistics for percent differences between original M11 and M12 values against "reconstructed" values as dumped by the correction workflow.
"""
import logging
import os
import dask
...
def collect_info(granule_path, s3):
    """Collect granule percent differences for original and newly reconstructed M11/M12 workflow"""
            ...
            with xr.open_dataset(each_fhandle, engine=NC_ENGINE) as each_ds:
                ...
                with s3.open(each_orig_path, mode='rb') as each_orig_fhandle:
                        with xr.open_dataset(each_orig_fhandle, engine=NC_ENGINE) as each_orig_ds:
                            ...
                            # compute M* percent diffs
                            each_M11_diff = each_ds.M11 - each_orig_ds.M11
                            each_M12_diff = each_ds.M12 - each_orig_ds.M12

                            each_M11_avg = (each_ds.M11 + each_orig_ds.M11)*0.5
                            each_M12_avg = (each_ds.M12 + each_orig_ds.M12)*0.5

                            each_M11 = 100*each_M11_diff/each_M11_avg
                            each_M12 = 100*each_M12_diff/each_M12_avg
                            ...
                            _min_M11 = abs(each_M11.min().item())
                            _min_M12 = abs(each_M12.min().item())

                            _max_M11 = abs(each_M11.max().item())
                            _max_M12 = abs(each_M12.max().item())

                            return (_mean_M11, _mean_M12, _std_M11, _std_M12, _min_M11, _min_M12, _max_M11, _max_M12, granule_path)
...
if __name__ == '__main__':
    ...
    while num_to_fix > 0:
        ...
        for each_result in results[0]:
            ...
            each_mean_M11, each_mean_M12, each_std_M11, each_std_M12, each_min_M11, each_min_M12, each_max_M11, each_max_M12, granule_path = each_result
            ...
            min_M11.append(each_min_M11)
            min_M12.append(each_min_M12)
            max_M11.append(each_max_M11)
            max_M12.append(each_max_M12)
            all_granules_paths.append(granule_path)
    ...
    results = {
        ...
        "min_M12": min_M12,
        "max_M12": max_M12,
        "all_granules_paths": all_granules_paths
    }

    output_file = 'validation_results_percent_diff.json'
