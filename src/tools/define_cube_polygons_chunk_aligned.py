"""
Define datacube polygons that are chunk aligned for production.
...
def define_cubes(shape_filename: str, cube_filename: str, target_epsg_codes: list, grid_size: int):
    ...
    for index, each_row in shapefile.iterrows():
        logging.info(f'Processing row {index} with EPSG {each_row.epsg} and ROI {each_row.ROI}')
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help='Regional shape file that defines each of the EPSG polygons in '
            'longitude/latitude coordinates [%(default)s].'
    )
    parser.add_argument(
        '-o', '--outputFile',
        type=str,
